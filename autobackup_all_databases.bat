...
SET myDIR=%backup_path%\all_databases
if not EXIST %myDIR% (MKDIR %myDIR%)

REM execute
%mysql_xampp% -u%mysql_username% -p%mysql_password% --all-databases --routines --events > %myDIR%\%filename%.sql

REM log information
if %ERRORLEVEL% neq 0 (
    (echo JAM=%TIME% # TANGGAL=%DATE% # Backup Semua Database Gagal) >> "%myLOG%\mysql_backup_log.txt"
) else (echo JAM=%TIME% # TANGGAL=%DATE% # Backup Semua Database Sukses) >> "%myLOG%\mysql_backup_log.txt"